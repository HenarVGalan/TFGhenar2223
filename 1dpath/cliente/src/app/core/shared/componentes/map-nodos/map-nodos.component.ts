...
export class MapNodosComponent implements OnInit {
  ...
  viasgroupGeoJson = new L.LayerGroup();
  tramosRutagroupGeoJson = new L.LayerGroup();
  tramosañadidos = new L.LayerGroup();
  ...
  public reset(): void {
    ...
    this.makePuntoMarkers();
    this.tramosañadidos.clearLayers();
  ...
  getDataFromApi(): void {
    ...
    this.tramosañadidos.addLayer(L.polyline(geojson29.coordinates[0].map(coordinate => [coordinate[1], coordinate[0]]), { weight: 4, color: 'blue' }));
    this.tramosañadidos.addLayer(L.polyline(geojson291.coordinates[0].map(coordinate => [coordinate[1], coordinate[0]]), { weight: 4, color: 'blue' }));
    this.tramosañadidos.addTo(this.mapa);
  }
